// NOTE: THIS IS UNUSED CODE FOR NOW!!!
// IT MAKES THE DEPLOY PROCESS SLOWER but allows the user the ability to call a command in the middle before the zip file is prepared
...
const helpers = {
    ...
    files: {
        createDeployFolder({ projectName, buildFiles: files, buildDirs: folders, buildRoot: root, version, userCommand }: CreateDeployParams) {
            ...
            const copiedFolders = folders.map(folder => {
                ...
                const copyDest = path.resolve(PATHS.deploy.root, relativePath)
                console.log(`    ${folder} -> ${copyDest}`)
                scripts.helpers.file.copyDir(folder, copyDest, false)
                return copyDest
            })
            ...
            const copiedFiles = files.map(file => {
                ...
                const copyDest = path.resolve(PATHS.deploy.root, relativePath)
                console.log(`    ${file} -> ${copyDest}`)
                scripts.helpers.file.copyFile(file, copyDest)
                return copyDest
            })

            // run any commands now that the user may have specified
            if (userCommand) {
                console.log(`Running user command: ${userCommand}`)
                const cwd = process.cwd()
                process.chdir(PATHS.deploy.root)
                runCommand(userCommand, { silent: false })
                process.chdir(cwd)
            }
            ...
            copiedFolders.forEach(folder => {
                const relativePath = path.relative(PATHS.deploy.root, folder)
            ...
            copiedFiles.forEach(file => {
                const fileFolder = file
                    ...
                    .slice(0, -1)
                    .join(path.sep)
                const relativePath = path.relative(PATHS.deploy.root, fileFolder)
            ...
            console.log(`Writing package.json for build... ${PATHS.deploy.packageJson}`)
...
export async function herokuDeployNode({
    ...
    helpers.files.createDeployFolder({
        ...
        userCommand: userCommandAfterCopy || ""
    })
